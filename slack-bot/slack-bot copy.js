...
const FEEDBACKFLOW_API_BASE = process.env.FEEDBACKFLOW_API_BASE || 'https://api.feedbackflow.app/api';
...
async function analyzeFeedback(url) {
  ...
  if (!FEEDBACKFLOW_API_KEY) {
    throw new Error('FeedbackFlow API key is not configured. Set FEEDBACKFLOW_API_KEY in your environment.');
  }

  // Use AutoAnalyze for general analysis. For platform-specific endpoints, callers can be adapted.
  const apiUrl = `${FEEDBACKFLOW_API_BASE.replace(/\/+$/, '')}/AutoAnalyze`;

  const params = Object.assign({}, getApiParams(platform, url));
  // default type 0 = analysis only
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      const response = await axios.get(apiUrl, {
        params,
        headers: {
          'x-api-key': FEEDBACKFLOW_API_KEY,
        },
        timeout: 120000,
        validateStatus: () => true, // we'll handle statuses
      });
      ...
      if (response.status === 429 || response.status === 503 || response.status === 500) {
        lastError = new Error(`API Error ${response.status}: ${JSON.stringify(response.data)}`);
  ...
  throw lastError || new Error('Unknown error calling FeedbackFlow API');
}
